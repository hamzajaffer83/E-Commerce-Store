...
const Table = ({ filteredCategories, handleCategoryDelete }: { filteredCategories: any[]; handleCategoryDelete: (id: number) => void }) => {
    return (
                        ...
                        filteredCategories.map((category) => (
                            <tr key={category.id} className="border-b transition-all hover:bg-gray-500/10 dark:hover:bg-white/10">
                                ...
                                <td className="px-4 py-2">
                                    <Link href={route('admin.category.sub.index', { parent_id: category.id })} className="hover:underline">
                                        {category.name}
                                    </Link>
                                </td>
                                <td className="px-4 py-2">
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent>
                                            <DropdownMenuItem>
                                                <Link href={route('admin.category.edit', category.id)}>Edit</Link>
                                            </DropdownMenuItem>
                                            <DropdownMenuItem>
